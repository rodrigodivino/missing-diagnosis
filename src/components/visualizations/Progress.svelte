...
<g class="outerRing" transform="translate({x * $canvasWidth}, {y * $canvasHeight})">
    <g class="marginConvention" transform="translate({margin.left},{margin.top})">
        <rect class="background" width={innerWidth} height={innerHeight}></rect>
        <rect class="foreground" width={innerWidth*progress} height={innerHeight}></rect>
        <text class="label" x={innerWidth*progress} y={innerHeight/2}> {(progress*100).toFixed(2)}% </text>
    </g>
</g>
<style>
    rect.background {
